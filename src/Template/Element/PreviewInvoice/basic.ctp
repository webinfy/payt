...
$colspan = $payment->status == 1 ? 4 : 3;
?>
<div class="main-content">
    <h1 style="text-align: center;">Pay Online for <span class="blue-color"><?= $payment->webfront->url; ?></span></h1>
    <div class="payment-table-section payment-page">

        <!--<div class="important-links">
            <a href="<?= HTTP_ROOT ?>"> Go Back To Dashboard</a> 
        ...
        <table class="payment-table" width="100%" cellpadding="0" cellspacing="0">
            ...
            <tr>
                ...
                <td><?= $this->Custom->dateDisplay($payment->uploaded_payment_file->payment_cycle_date); ?></td>
            ...
            if ($paymentFieldCount > 0) {
                foreach ($paymentCustomFields as $paymentCustomField) {
                    ?>
                    <tr>
                        <td colspan="<?= $colspan ?>"><?= $paymentCustomField['field'] ?></td>
            ...
            <div class="btn-section text-center">
                ...
                if (!empty($payment->webfront->user->merchant_payment_gateway)) {
                    if ($payment->webfront->user->merchant_payment_gateway->payment_gateway->id == 2) {
                        ...
                        <p> <label><?= $this->Form->checkbox('is_agree', ['required' => TRUE]); ?> Accept Term & Conditions</label></p>
                        <script
                    ...
                    <?php } else if ($payment->webfront->user->merchant_payment_gateway->payment_gateway->id == 1) { ?>
                        ...
                        <form action='<?= $payuUrl ?>' method='post' style="display: inline;">
                            ...
                            <p style="text-align: center;"> <label><?= $this->Form->checkbox('is_agree', ['required' => TRUE]); ?> Accept Term & Conditions</label></p>
